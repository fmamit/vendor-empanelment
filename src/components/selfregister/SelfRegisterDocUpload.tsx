...
interface SelfRegisterDocUploadProps {
...
export function SelfRegisterDocUpload({ categoryDocs, sessionId, uploadedDocs, onDocUploaded }: SelfRegisterDocUploadProps) {
  ...
  const handleUpload = async (documentTypeId: string, file: File) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("upload-self-register-document", {
        body: formData,
      });
